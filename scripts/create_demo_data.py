...
def create_demo_data():
    ...
    for hospital in hospitals:
        response = requests.post(f"{BASE_URL}/hospitals", json=hospital)
    ...
    for i, study in enumerate(studies):
        ...
        response = requests.post(f"{BASE_URL}/hospitals/{hospital_id}/studies", json=study)
        if response.status_code == 201:
            print(f"Created study at {hospital_id}: {study['description']}")
    
    print("\n✅ Demo data created successfully!")
    print(f"\nTest the federation:")
    print(f"1. Visit: http://localhost:5000/federation-access?national_id=199012345678901234")
    print(f"2. Visit: http://localhost:5000/dashboard")
    print(f"3. Test QR codes for patient: {patient_ids[0]}")
